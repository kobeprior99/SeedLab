...
def check_arrow(masks, frame, aruco_center):
    ...
    for contour in green_contours:
        if cv2.contourArea(contour) > 500:
            ...
            if x < aruco_center[0] and ((aruco_center[0] - 100) < (x + (w/2))) and (y + (h/2))< aruco_center[1] + 100 and (y + (h/2)) > aruco_center[1] - 100:
    ...
    for contour in red_contours:
        if cv2.contourArea(contour) > 500:
            ...
            if x > aruco_center[0] and ((x + (w/2)) < (aruco_center[0] + 100)) and (y + (h/2)) < aruco_center[1] + 100 and (y + (h/2)) > aruco_center[1] - 100:
                cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
