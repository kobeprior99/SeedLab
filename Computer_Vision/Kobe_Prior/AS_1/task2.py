#Each of the state functions below performs tasks depicteced in FSM in INTRO-to-Pi assignment
...
def state_c(current_char):
    if current_char == 'd':
        ...
        return state_0
#note that state_d is the final state and saves additional iterations in for loop since the task is to see if the 'abcd' sequence is present in the string
...
def finite_state_machine_test(string):
    ...
    for char in string:
        ...
        if state == None:
            return 
    #if you made it out of the loop without returning and the current state isn't None, then the string does not contain 'abcd' sequence
